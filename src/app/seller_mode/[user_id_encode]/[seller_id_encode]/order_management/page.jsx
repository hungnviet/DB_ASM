...
export default function ({ params }) {
  ...
  async function fetchData() {
    const response = await fetch(`/api/order_seller?seller_id=${seller_id}`);
  ...
  async function onClick(index) {
    if (indexUpdate === index) {
      ...
      const data = {
        ...
        State: order[index].State,
        Expected_date: new Date(order[index].Expected_date)
          .toISOString()
          .split("T")[0],
        Creation_date: new Date(order[index].Creation_date)
          .toISOString()
          .split("T")[0],
      };
      console.log(data);
      const response = await fetch("/api/order_seller", {
        method: "PUT",
        headers: {
  ...
  function handleChangeState(event, index) {
    ...
    newOrder[index].State = event.target.value;
    setOrder(newOrder);
  }

  function handleChangeDateExpected(event, index) {
    const newOrder = [...order];
    newOrder[index].Expected_date = event.target.value;
    setOrder(newOrder);
  }
  function handleChangeDateCreation(event, index) {
    const newOrder = [...order];
    newOrder[index].Creation_date = event.target.value;
    setOrder(newOrder);
  }
  return (
                  ...
                  <td>
                    {indexUpdate === index ? (
                      <input
                        type="Date"
                        value={
                          new Date(order[index].Creation_date)
                            .toISOString()
                            .split("T")[0]
                        }
                  ...
                  <td>
                    {indexUpdate === index ? (
                      <input
                        type="Date"
                        value={
                          new Date(order[index].Expected_date)
                            .toISOString()
                            .split("T")[0]
                        }
